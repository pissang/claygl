...
function App3D(dom, appNS) {
  ...
  ['alpha', 'depth', 'stencil', 'antialias', 'premultipliedAlpha', 'preserveDrawingBuffer'].forEach(
    function (attrName) {
  ...
  gScene.on(
    'beforerender',
    function (renderer, scene, camera, renderList) {
      if (this._inRender) {
...
App3D.prototype._initMouseEvents = function (rayPicking) {
  ...
  EVE_NAMES.forEach(function (_eveType) {
    vendor.addEventListener(
      dom,
      _eveType,
      (this[makeHandlerName(_eveType)] = function (e) {
        ...
        if (pickResult) {
          ...
          if (eveType === 'mousemove') {
            ...
            bubblingEvent(
              pickResult.target,
              packageEvent('mousemove', pickResult, offsetX, offsetY)
            );
            if (targetChanged) {
              bubblingEvent(
                pickResult.target,
                packageEvent('mouseover', pickResult, offsetX, offsetY)
              );
            }
          } else {
            bubblingEvent(
 ...
 */
App3D.prototype.createSpotLight = function (
  position,
  target,
  range,
  color,
 ...
 * @return {Promise}
 */
App3D.prototype.createAmbientCubemapLight = function (
  envImage,
  specIntensity,
  diffIntensity,
  exposure,
